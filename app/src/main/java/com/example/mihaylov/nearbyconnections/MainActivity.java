...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks {
    ...
    public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
        logW(
                String.format(
                        ...
                        new Status(connectionResult.getErrorCode())));
    }
        ...
        if (requestCode == REQUEST_CODE_REQUIRED_PERMISSIONS) {
            for (int grantResult : grantResults) {
                if (grantResult == PackageManager.PERMISSION_DENIED) {
                    Toast.makeText(this, "error missing permisssions ", Toast.LENGTH_LONG).show();
                    finish();
                    return;
                }
            }
                        ...
                        new ResultCallback<Status>() {
                            ...
                            public void onResult(@NonNull Status status) {
                                if (!status.isSuccess()) {
                                    logW(
                                            String.format(
                                                    "rejectConnection failed. %s", status));
                                }
                            }
                        });
    }
                        ...
                        new ResultCallback<Status>() {
                            ...
                            public void onResult(@NonNull Status status) {
                                if (!status.isSuccess()) {
                                    logW(
                                            String.format(
                                                    "requestConnection failed. %s", status));
                                    mIsConnecting = false;
    ...
    protected String getServiceId() {
        return getString(R.string.service_id);
    }
